...
| # | Problem | Difficulty | Status | Hint / One-Liner | Approaches | My Notes |
|---|---------|-----------|--------|-------------------|------------|----------|
| 1 | [Best Time to Buy and Sell Stock](https://leetcode.com/problems/best-time-to-buy-and-sell-stock/) | 🟢 Easy | ✅ | Not really sliding window — track running min price; profit = price - minSoFar | Track min O(n) · Brute O(n²) | Buy = minSoFar pointer, Sell = right pointer; update max profit each step |
| 2 | [Longest Substring Without Repeating Characters](https://leetcode.com/problems/longest-substring-without-repeating-characters/) | 🟡 Medium | ✅ | Window must always hold unique chars; if new char duplicates, shrink from left until valid | Sliding window + Set O(n) | Keep shrinking left while duplicate exists; update maxLen after each valid expansion |
...
| # | Problem | Difficulty | Status | Hint / One-Liner | Approaches | My Notes |
|---|---------|-----------|--------|-------------------|------------|----------|
| 1 | [Binary Search](https://leetcode.com/problems/binary-search/) | 🟢 Easy | ✅ | Classic lo/hi/mid — move lo or hi based on comparison with target | Iterative O(log n) · Recursive O(log n) | |
| 2 | [Search a 2D Matrix](https://leetcode.com/problems/search-a-2d-matrix/) | 🟡 Medium | ✅ | Start from top-right corner; go left if too big, go down if too small | Staircase traversal O(m+n) · Binary on flattened matrix O(log mn) | Top-right is the pivot: only corner that lets you eliminate a row or column each step |
| 3 | [Koko Eating Bananas](https://leetcode.com/problems/koko-eating-bananas/) | 🟡 Medium | ✅ | Binary search on the answer (k range); check if hours(k) ≤ h | Binary on answer space O(n log maxPile) | ceil(pile/k) per pile to compute hours; lo=1, hi=max(piles) |
| 4 | [Find Minimum in Rotated Sorted Array](https://leetcode.com/problems/find-minimum-in-rotated-sorted-array/) | 🟡 Medium | ✅ | Array has two sorted segments; left segment always > right segment — binary search for the smallest element on the right segment | Binary O(log n) | If mid > right → min is in right half; else min is in left half (including mid) |
| 5 | [Search in Rotated Sorted Array](https://leetcode.com/problems/search-in-rotated-sorted-array/) | 🟡 Medium | ✅ | Determine which half is sorted, then check if target lies within that sorted half | Binary O(log n) | One half is always sorted; use that to decide which side to search |
| 6 | [Time Based Key-Value Store](https://leetcode.com/problems/time-based-key-value-store/) | 🟡 Medium | ✅ | HashMap with key → list of (timestamp, value); binary search for largest timestamp ≤ t | HashMap + Binary search O(log n) per get | Timestamps are inserted in order so list stays sorted; bisect_right - 1 gives the answer |
| 7 | [Median of Two Sorted Arrays](https://leetcode.com/problems/median-of-two-sorted-arrays/) | 🔴 Hard | ✅ | Naive: merge with two pointers O(m+n); Optimal: binary search partition on smaller array | Two-ptr O(m+n) · Binary partition O(log min(m,n)) | Partition so left halves combined = right halves combined; check maxLeft ≤ minRight |
